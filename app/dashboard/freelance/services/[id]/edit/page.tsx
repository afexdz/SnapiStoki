        ...
        return (
          <div key={num} className="flex items-center flex-1">
            <div className="flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-colors ${done || active ? "bg-[#FA8112] border-[#FA8112] text-white" : "bg-white border-gray-300 text-gray-400"}`}>
              ...
              <span className={`hidden sm:block text-[10px] mt-1 whitespace-nowrap font-medium ${active ? "text-[#FA8112]" : "text-gray-400"}`}>{label}</span>
            </div>
            {i < steps.length - 1 && <div className={`flex-1 h-0.5 sm:mb-4 mx-1 ${num < step ? "bg-[#FA8112]" : "bg-gray-200"}`} />}
          </div>
        )
      })}
    </div>
      ...
      <main className="min-h-screen bg-[#FAFAFA] py-8 px-4">
        <div className="max-w-3xl mx-auto">
          <div className="mb-6 flex items-center gap-4">
            <Link href="/dashboard/freelance/services" className="p-2 rounded-xl border border-[var(--ink-12)] text-gray-500 hover:text-[#FA8112] hover:border-[#FA8112]/40 transition-all">
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <div onClick={() => setSinglePkg(p => !p)} className={`relative w-10 h-5 rounded-full transition-colors cursor-pointer ${singlePkg ? "bg-[#FA8112]" : "bg-gray-200"}`}>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {faqItems.length < 5 && <button onClick={() => setFaqItems(p => [...p, { q: "", a: "" }])} className="text-sm text-[#FA8112] font-semibold hover:text-[#E8730F]">+ Ajouter une question</button>}
                  </div>
            ...
            {step === 4 && (
              <div className="space-y-6">
                <div>
                  ...
                  {totalImages < 3 && (
                    <div ref={dropRef} onDrop={onDrop} onDragOver={e => e.preventDefault()} className="border-2 border-dashed border-[var(--ink-12)] rounded-xl p-8 text-center cursor-pointer hover:border-[#FA8112]/40 transition-colors" onClick={() => document.getElementById("edit-img-input")?.click()}>
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-3">
                      {existingImages.map((img, i) => (
                        <div key={img.url} className="relative aspect-video bg-gray-100 rounded-xl overflow-hidden">
                          ...
                          {i === 0 && <span className="absolute bottom-1.5 left-1.5 px-1.5 py-0.5 bg-[#FA8112] text-white text-[9px] font-bold rounded">Cover</span>}
                        </div>
                      ))}
                      {newImages.map((img, i) => (
          ...
          <div className="flex items-center justify-between mt-6">
            ...
            {step < 4 ? (
              <button onClick={() => setStep(s => s + 1)} disabled={(step === 1 && !step1Valid) || (step === 2 && !step2Valid) || (step === 3 && !step3Valid)} className="px-6 py-2.5 bg-[#FA8112] text-white text-sm font-bold rounded-xl hover:bg-[#E8730F] transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                Continuer →
              </button>
            ) : (
              <button onClick={handleSubmit} disabled={submitting || totalImages === 0} className="px-6 py-2.5 bg-[#FA8112] text-white text-sm font-bold rounded-xl hover:bg-[#E8730F] transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-2">
