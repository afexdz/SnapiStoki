...
function FilterPanel({
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex gap-2 mb-3">
          ...
          <div className="flex-1">
            ...
            <div className="relative">
              <input type="number" value={maxInput} onChange={e => handleMaxInput(e.target.value)} placeholder="Illimité" min={0} className={PRICE_INPUT_CLS} />
              <span className="absolute right-2 inset-y-0 flex items-center text-xs text-gray-400 pointer-events-none">DA</span>
            </div>
          </div>
        </div>
        {priceRangeError && <p className="text-xs text-red-500 mb-2">Le minimum ne peut pas dépasser le maximum.</p>}
      ...
      <button onClick={handleReset} className="w-full py-2.5 text-sm text-gray-500 dark:text-gray-400 hover:text-[var(--orange)] border border-[var(--border-subtle)] dark:border-[var(--border-subtle)] rounded-xl transition-colors">
...
export default function FreelancesClient({
  services,
  categories,
  activeCategorie,
}: {
  ...
  const [category, setCategory]       = useState(activeCategorie)
  ...
  // Category change → shareable URL (server re-fetches)
  function handleCategoryChange(c: string) {
    setCategory(c)
    startTransition(() => {
      const url = c === "all" ? "/freelances" : `/freelances?categorie=${encodeURIComponent(c)}`
  ...
  const handleMinInput = (raw: string) => {
    ...
    minTimerRef.current = setTimeout(() => {
      setMinPrice(raw === "" ? 0 : Math.max(0, parseInt(raw, 10) || 0))
    }, 400)
  }
  ...
  const handleMaxInput = (raw: string) => {
    ...
    maxTimerRef.current = setTimeout(() => {
      setMaxPrice(raw === "" ? 0 : Math.max(0, parseInt(raw, 10) || 0))
    }, 400)
  }
  ...
  const filtered = useMemo(() => {
    ...
    return list
  }, [services, category, minRating, minPrice, maxPrice, sort, userWilaya])

  const filterPanelProps: FilterPanelProps = {
    categories, category, setCategory: handleCategoryChange,
    minRating, setMinRating,
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-500 dark:text-gray-400">
              {isPending ? "Chargement…" : <><strong className="text-[var(--ink)] dark:text-[var(--ink)]">{filtered.length}</strong> services trouvés</>}
            </span>
            <button
