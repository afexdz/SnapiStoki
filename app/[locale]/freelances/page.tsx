...
function FilterPanel({
  ...
  const t = useTranslations("freelances")

  return (
    <div className="space-y-6">
      {/* Category */}
      <div>
        <h3 className="text-sm font-semibold text-[var(--ink)] dark:text-[var(--ink)] mb-3">{t("panel.category")}</h3>
        <div className="space-y-1.5">
          <button
            onClick={() => setCategory("all")}
            className={`w-full text-start px-3 py-2 rounded-lg text-sm transition-colors ${category === "all" ? "bg-[var(--orange)] text-white font-medium" : "text-gray-600 dark:text-gray-400 hover:bg-[var(--cream)] dark:hover:bg-[#2a2a2a] hover:text-[var(--orange)]"}`}
          >
          ...
          {categories.map(c => (
            <button
              ...
              className={`w-full text-start px-3 py-2 rounded-lg text-sm transition-colors ${category === c ? "bg-[var(--orange)] text-white font-medium" : "text-gray-600 dark:text-gray-400 hover:bg-[var(--cream)] dark:hover:bg-[#2a2a2a] hover:text-[var(--orange)]"}`}
            >
              {c}
            </button>
          ))}
      ...
      {/* Min rating */}
      <div>
        <h3 className="text-sm font-semibold text-[var(--ink)] dark:text-[var(--ink)] mb-3">{t("panel.minRating")}</h3>
        <div className="space-y-1.5">
          {[0, 4, 4.5, 4.8].map(r => (
            <button
              key={r}
            ...
            >
              {r === 0 ? t("panel.allRatings") : <><Stars rating={r} /><span>{r}+</span></>}
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Budget */}
      <div>
        <h3 className="text-sm font-semibold text-[var(--ink)] dark:text-[var(--ink)] mb-3">{t("panel.budget")}</h3>
        <div className="flex gap-2 mb-3">
          <div className="flex-1">
            <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">{t("panel.min")}</label>
            <div className="relative">
              <input type="number" value={minInput} onChange={e => handleMinInput(e.target.value)} placeholder="0" min={0} className={PRICE_INPUT_CLS} />
              <span className="absolute end-2 inset-y-0 flex items-center text-xs text-gray-400 pointer-events-none">DA</span>
            </div>
          </div>
          <div className="flex-1">
            <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">{t("panel.max")}</label>
            <div className="relative">
              <input type="number" value={maxInput} onChange={e => handleMaxInput(e.target.value)} placeholder={t("panel.unlimited")} min={0} className={PRICE_INPUT_CLS} />
              <span className="absolute end-2 inset-y-0 flex items-center text-xs text-gray-400 pointer-events-none">DA</span>
            </div>
          </div>
        </div>
        {priceRangeError && <p className="text-xs text-red-500 mb-2">{t("panel.priceError")}</p>}
        <input type="range" min={1000} max={SLIDER_MAX} step={1000} value={sliderValue} onChange={e => handleSlider(Number(e.target.value))} className="w-full accent-[#FA8112]" />
        <div className="flex justify-between text-xs text-gray-400 mt-1">
          <span>1 000 DA</span>
          <span>{maxPrice === 0 ? t("panel.unlimited") : `${maxPrice.toLocaleString()} DA`}</span>
        </div>
      </div>
...
export default function FreelancesPage() {
  ...
  const filterPanelProps: FilterPanelProps = {
    categories, category, setCategory,
    minRating, setMinRating,
    minInput, handleMinInput, maxInput, handleMaxInput,
    priceRangeError, sliderValue, handleSlider, maxPrice,
    handleReset,
  }
        ...
        <div className="bg-[var(--cream)] dark:bg-[var(--white)] border-b border-[var(--border-subtle)] dark:border-[var(--border-subtle)]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-3">
              <Link href="/" className="hover:text-[var(--orange)] transition-colors">{t("breadcrumb.home")}</Link>
              <span>/</span>
              <span className="text-[var(--ink)] dark:text-[var(--ink)]">{t("breadcrumb.services")}</span>
            </div>
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold text-[var(--ink)] dark:text-[var(--ink)]">{t("hero.title")}</h1>
                <p className="text-gray-500 dark:text-gray-400 mt-1">{t("hero.subtitle")}</p>
              </div>
              <Link
                href="/services/new"
                className="px-4 py-2.5 bg-[var(--orange)] text-white text-sm font-bold rounded-xl hover:bg-[var(--orange-dark)] transition-colors shadow-md shadow-[var(--orange)]/30"
              >
                {t("hero.publishCta")}
              </Link>
            </div>
            ...
            <div className="flex items-center gap-3">
              <span className="text-sm text-gray-500 dark:text-gray-400">
                {loading
                  ...
                  : <strong className="text-[var(--ink)] dark:text-[var(--ink)]">{t("count", { n: filtered.length })}</strong>
                }
              </span>
              <button
          ...
          <div className="flex gap-8">
            ...
            <aside className="hidden lg:block w-64 shrink-0">
              <div className="sticky top-6 bg-[var(--white)] dark:bg-[var(--surface-3)] rounded-2xl border border-[var(--border-subtle)] dark:border-[var(--border-strong)] p-5 shadow-sm dark:shadow-[var(--shadow-elevation)]">
                <h2 className="font-semibold text-[var(--ink)] dark:text-[var(--ink)] mb-5">{t("panel.title")}</h2>
            ...
            {sidebarOpen && (
              <div className="fixed inset-0 z-50 lg:hidden">
                ...
                <div className="absolute start-0 top-0 bottom-0 w-72 bg-[var(--white)] dark:bg-[var(--white)] overflow-y-auto p-5 shadow-2xl">
                  <div className="flex items-center justify-between mb-5">
                    <h2 className="font-semibold text-[var(--ink)] dark:text-[var(--ink)]">{t("panel.title")}</h2>
            ...
            <div className="flex-1 min-w-0">
              {loading ? (
                ...
                <div className="text-center py-20">
                  ...
                  <h3 className="text-lg font-semibold text-[var(--ink)] dark:text-[var(--ink)] mb-1">{t("empty.title")}</h3>
                  <p className="text-gray-500 dark:text-gray-400 text-sm">{t("empty.hint")}</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
