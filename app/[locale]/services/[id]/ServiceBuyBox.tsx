      ...
      <div className="bg-[var(--white)] rounded-2xl border border-[var(--orange)]/30 p-5 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-3 bg-gray-50 dark:bg-[var(--white)] rounded-xl text-center">
            ...
            <p className="text-xs text-gray-500">{t("owner.orders")}</p>
          </div>
          <div className="p-3 bg-gray-50 dark:bg-[var(--white)] rounded-xl text-center">
            <p className="text-lg font-black text-[var(--ink)]">{coverPrice.toLocaleString()}</p>
            <p className="text-xs text-gray-500">{t("owner.startingAt")}</p>
          </div>
        </div>
    ...
    <div className="bg-[var(--white)] rounded-2xl border border-[var(--border-subtle)] p-5 shadow-sm">
        ...
        <div className="flex rounded-xl overflow-hidden border border-[var(--border-subtle)] mb-4">
          {PKG_ORDER.filter(k => packages && k in packages).map(k => (
            <button
              ...
              className={`flex-1 py-2 text-xs font-semibold transition-colors ${activeTab === k ? "bg-[var(--orange)] text-white" : "bg-[var(--white)] text-gray-600 hover:bg-gray-50"}`}
            >
              {t(`tab.${k}` as "tab.basique" | "tab.standard" | "tab.premium")}
            </button>
          ))}
        </div>
      )}
        ...
        <>
          ...
          <div className="flex gap-4 mb-4 text-xs text-gray-600">
            <div className="flex items-center gap-1">
              ...
              {t("delivery", { n: pkg.delivery_days })}
            </div>
            <div className="flex items-center gap-1">
              <svg className="w-3.5 h-3.5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
              {pkg.revisions === -1 ? t("revisions.unlimited") : t("revisions.count", { n: pkg.revisions })}
            </div>
          </div>
        </>
      )}
      ...
      <div className="flex items-end justify-between mb-4">
        <div>
          ...
          <p className="text-2xl font-bold text-[var(--ink)]">{coverPrice.toLocaleString()} <span className="text-base text-[var(--orange)]">DA</span></p>
        </div>
        {!pkg && (
