    ...
    async function load() {
      ...
      const { data: svc, error: svcErr } = await sb
        ...
        .select(`
          ...
          seller:profiles!services_seller_id_fkey(id, full_name, avatar_url, wilaya, bio, created_at, whatsapp)
        `)
        .eq("id", id)
        .single()
                ...
                {isOwner ? (
                  ...
                  <div className="bg-[var(--white)] rounded-2xl border border-[var(--border-subtle)] p-5 shadow-sm">
                    ...
                    {service.seller?.whatsapp ? (
                      <a
                        href={`https://wa.me/${service.seller.whatsapp.replace(/[^\d]/g, "")}?text=${encodeURIComponent(`Bonjour, je suis intéressé par votre service « ${service.title} » sur PixRaise.`)}`}
                        target="_blank"
          ...
          <>
            ...
            {service.seller?.whatsapp ? (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 flex-1 max-w-[220px] py-3 bg-[var(--orange)] text-white font-bold text-sm rounded-xl hover:bg-[var(--orange-dark)] transition-colors"
              >
