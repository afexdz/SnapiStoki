              ...
              <div className="relative" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 top-full mt-2 w-52 bg-white dark:bg-[var(--white)] rounded-2xl border border-[rgba(26,26,26,0.12)] dark:border-[var(--ink-12)] shadow-xl shadow-gray-200/60 dark:shadow-black/40 py-1.5 z-50">
                    ...
                    <Link href="/profile" onClick={() => setDropdownOpen(false)} className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-[rgba(26,26,26,0.7)] dark:text-gray-300 hover:bg-[var(--orange-soft)] hover:text-[#FA8112] transition-colors">
                    ...
                    <div className="border-t border-[rgba(26,26,26,0.08)] dark:border-[var(--ink-12)] mt-1 pt-1">
                      {seller ? (
                        <>
                          ...
                          <Link href="/products/new" onClick={() => setDropdownOpen(false)} className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-[rgba(26,26,26,0.7)] dark:text-gray-300 hover:bg-[var(--orange-soft)] hover:text-[#FA8112] transition-colors">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
                            Vendre un produit
                          </Link>
                        </>
                      ) : (
                        <Link href="/devenir-vendeur" onClick={() => setDropdownOpen(false)} className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-[rgba(26,26,26,0.7)] dark:text-gray-300 hover:bg-[var(--orange-soft)] hover:text-[#FA8112] transition-colors">
          ...
          <div className="md:hidden pb-4 space-y-1 border-t border-[rgba(26,26,26,0.08)] dark:border-[#2a2a2a] pt-3 animate-slide-down">
            {navLinks.map((link) => (
              <Link
                ...
                className="flex items-center px-3 py-3 text-sm font-medium text-[rgba(26,26,26,0.7)] dark:text-gray-400 hover:text-[#1A1A1A] dark:hover:text-[#FAF3E1] hover:bg-[var(--orange-soft)] dark:hover:bg-[#2a2a2a] rounded-[10px] transition-colors font-jakarta min-h-[44px]"
              >
                {link.label}
              </Link>
            ))}
            {user ? (
              <>
                <Link href="/dashboard" onClick={() => setMenuOpen(false)} className="flex items-center px-3 py-3 text-sm font-medium text-[rgba(26,26,26,0.7)] dark:text-gray-400 hover:text-[#FA8112] rounded-[10px] hover:bg-[var(--orange-soft)] dark:hover:bg-[#2a2a2a] transition-colors font-jakarta min-h-[44px]">
                  Tableau de bord
                </Link>
                {seller ? (
                  <>
                    <Link href="/services/new" onClick={() => setMenuOpen(false)} className="flex items-center px-3 py-3 text-sm font-medium text-[rgba(26,26,26,0.7)] dark:text-gray-400 hover:text-[#FA8112] rounded-[10px] hover:bg-[var(--orange-soft)] dark:hover:bg-[#2a2a2a] transition-colors font-jakarta min-h-[44px]">
                      Publier un service
                    </Link>
                    <Link href="/products/new" onClick={() => setMenuOpen(false)} className="flex items-center px-3 py-3 text-sm font-medium text-[rgba(26,26,26,0.7)] dark:text-gray-400 hover:text-[#FA8112] rounded-[10px] hover:bg-[var(--orange-soft)] dark:hover:bg-[#2a2a2a] transition-colors font-jakarta min-h-[44px]">
                      Vendre un produit
                    </Link>
                  </>
                ) : (
                  <Link href="/devenir-vendeur" onClick={() => setMenuOpen(false)} className="flex items-center px-3 py-3 text-sm font-medium text-[#FA8112] rounded-[10px] hover:bg-[var(--orange-soft)] dark:hover:bg-[#2a2a2a] transition-colors font-jakarta min-h-[44px]">
                    Devenir vendeur
                  </Link>
                )}
